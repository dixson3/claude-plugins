...
jq -n --arg idx "$PLAN_IDX" '{
  "decision": "block",
  "reason": ("BLOCKED: Plan " + $idx + " is saved but not yet executing. To proceed: 1. Run /workflows:plan_to_beads to create beads. 2. Say '\''execute the plan'\'' to start. Or run /workflows:dismiss_gate to abandon.")
}'
