...
- Can be invoked directly: `/workflows:breakdown_task <task_id>`
...
For each new child:
```
/workflows:select_agent <child-id>
```
...
Breakdown: <task_id> — "<task title>"
  ...
  Agent assignments: 1 task assigned to chronicler_diary

  Children:
    1. <child-1> — "<title>" [ready]
    2. <child-2> — "<title>" [blocked by child-1]
    3. <child-3> — "<title>" [blocked by child-2]
    4. <child-4> — "<title>" [ready, agent:chronicler_diary]
```
