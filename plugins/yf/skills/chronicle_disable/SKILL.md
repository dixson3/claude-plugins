...
Report includes: list of open chronicle beads found, close status for each. If no open chronicles, reports nothing to disable.

After disable, chronicler remains installed — you can continue capturing with `/yf:chronicle_capture` and generate diaries with `/yf:chronicle_diary`. Differs from `/yf:chronicle_diary` in that it closes beads without generating diary files.
