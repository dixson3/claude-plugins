...
| REQ-034 | Plugin must enforce three-condition activation gate: (1) `.yoshiko-flow/config.json` exists, (2) `enabled: true` in config, (3) bd CLI available. All skills except `/yf:setup` refuse when any condition fails. | P0 | Core | Plan 42 | `/Users/james/workspace/dixson3/d3-claude-plugins/plugins/yf/scripts/yf-activation-check.sh` |
