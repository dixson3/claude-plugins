...
| REQ-016 | Formulas must define reusable multi-agent workflow templates with step dependencies and agent annotations | P1 | Formula | Plan 28 | `plugins/yf/formulas/` |
| REQ-017 | Formula dispatch loop must instantiate formulas as wisps, dispatch steps to specialized agents, and squash wisps on completion | P1 | Formula | Plan 28 | `plugins/yf/skills/formula_execute/SKILL.md` |
| REQ-018 | Agents within a formula must communicate via structured comments (FINDINGS, CHANGES, REVIEW, TESTS) on the parent task | P1 | Formula | Plan 28 | `plugins/yf/rules/yf-rules.md` |
| REQ-019 | Formula labels must be auto-assigned to plan tasks based on title keyword heuristics during task creation | P1 | Formula | Plan 29 | `plugins/yf/skills/formula_select/SKILL.md` |
| REQ-020 | Maximum formula nesting depth is 2 (reactive bugfix at depth+1) | P2 | Formula | Plan 29 | `plugins/yf/rules/yf-rules.md` |
| REQ-021 | Reactive bugfix formula must auto-spawn on REVIEW:BLOCK or test failures with retry budget and design-BLOCK exclusion | P2 | Formula | Plan 29 | `plugins/yf/skills/formula_execute/SKILL.md` |
| REQ-022 | Code-review qualification gate must run before plan completion, configurable as blocking/advisory/disabled | P2 | Formula | Plan 29 | `plugins/yf/skills/formula_qualify/SKILL.md` |
...
| REQ-026 | Closed tasks must be automatically pruned at plan completion (plan-scoped) and after git push (global). Orphaned gates (chronicle-gate, qualification-gate) and closed chronicle files must be cleaned at session boundaries (session_land, plan_intake). | P2 | Task Management | Plan 32, Plan 60 | `/Users/james/workspace/dixson3/d3-claude-plugins/plugins/yf/scripts/plan-prune.sh`, `session-prune.sh` |
...
- FS-043: Skill-level chronicle capture fires deterministically at decision points — verdicts, spec mutations, scope changes. Formula-level chronicle capture fires via `"chronicle": true` step flag on terminal formula steps. Write-capable formula agents capture plan deviations and unexpected discoveries directly via `yft_create`. Read-only agents signal chronicle-worthy content via `CHRONICLE-SIGNAL:` lines in structured comments, consumed by `formula_execute` Step 3j.
